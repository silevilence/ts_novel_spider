...
const STAGE_LABEL: Record<string, string> = {
  glossary_setup: '确认术语候选', glossary_translation: '确认术语译文', translating: '正文初翻', checking: '遗漏检查', reviewing: '审核校对', completed: '已完成',
};
...
function WorkflowStateGraphCompact({ task, detail, currentChapter, onSelect }: { task: RefinedTask; detail: RefinedTaskDetail; currentChapter: string; onSelect: (stage: RefinedTaskStage) => void }) {
  const stages: Array<{ id: RefinedTaskStage; label: string; position: { x: number; y: number } }> = [
    ...
    { id: 'checking', label: '遗漏检查', position: { x: 440, y: 190 } }, { id: 'reviewing', label: '审核校对', position: { x: 660, y: 190 } }, { id: 'completed', label: '完成', position: { x: 900, y: 190 } },
  ];
  ...
  edges.push(
    ...
    { id: 'reviewing-translating', source: 'reviewing', sourceHandle: 'source-top', target: 'translating', targetHandle: 'target-right', label: `修改 ↺ · 第 ${detail.progress.currentRound}/${task.modelConfig.maxReviewRounds} 轮`, type: 'smoothstep', style: { ...edgeStyle, stroke: '#ffb366' }, labelStyle: { fill: '#ffca87', fontWeight: 700 }, labelBgStyle: { fill: '#211611', fillOpacity: .92 }, markerEnd: { type: MarkerType.ArrowClosed, color: '#ffb366' } },
  );
