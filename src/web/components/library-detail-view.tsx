...
export function LibraryDetailView({ model, onOpenControl, onNotify }: LibraryDetailViewProps) {
  ...
  return (
    <ScrollspyProvider>
      ...
      <Modal opened={descriptionDialog !== null} onClose={() => setDescriptionDialog(null)} title={descriptionDialog?.title ?? ''} size="lg">
        <Text size="sm" style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{descriptionDialog?.text}</Text>
      </Modal>
