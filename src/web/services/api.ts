...
export interface RefinedModelConfigInput { termExtractionModel?: { providerId: string; modelId: string } | null; termTranslationModel?: { providerId: string; modelId: string } | null; translationModels?: Array<{ providerId: string; modelId: string }>; omissionModel?: { providerId: string; modelId: string } | null; reviewModel?: { providerId: string; modelId: string } | null; concurrency?: number; maxReviewRounds?: number; }
...
export async function fetchRefinedChapter(taskId: string, chapterId: string): Promise<{ chapter: { title: string }; segments: RefinedSegment[]; reviews: Array<{ id: string; severity: string; suggestion: string; replacementText: string | null; paragraphIndices: number[]; scores: Record<string, number>; resolved: boolean; resolution: RefinedReviewResolution; resolutionNote: string | null }> }> { return requestJson(`/api/refined-translations/tasks/${encodeURIComponent(taskId)}/chapters/${encodeURIComponent(chapterId)}`); }
...
export async function fetchRefinedReviews(taskId: string): Promise<{ reviews: Array<{ id: string; chapterId: string; severity: string; suggestion: string; replacementText: string | null; paragraphIndices: number[]; scores: Record<string, number>; resolved: boolean; resolution: RefinedReviewResolution; resolutionNote: string | null }> }> { return requestJson(`/api/refined-translations/tasks/${encodeURIComponent(taskId)}/reviews`); }
...
export async function extractRefinedTerms(taskId: string): Promise<{ terms: RefinedTerm[] }> { return requestJson(`/api/refined-translations/tasks/${encodeURIComponent(taskId)}/terms/extract`, { method: 'POST' }); }
